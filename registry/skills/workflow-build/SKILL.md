---
name: workflow-build
description: Workflow for executing production build compilation, validating
  bundle output integrity, asset sizes, and target artifacts. Use when executing
  production build verification workflows, phase transitions, and verification
  gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run build --if-present` | Compilation completes with exit code 0 and zero fatal errors |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npx agents-united --version` | Built CLI entry point executes cleanly without module resolution errors |
