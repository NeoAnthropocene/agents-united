...
description: Systematic troubleshooting workflow for diagnosing bugs, analyzing
  error logs, isolating root causes, and implementing verified fixes. Use when
  executing root cause diagnosis & troubleshooting workflows, phase transitions,
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Reproduction test passes cleanly post-fix |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run typecheck --if-present` | Type checker confirms patch introduces no type errors |
