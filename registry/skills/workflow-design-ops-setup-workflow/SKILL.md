---
name: workflow-design-ops-setup-workflow
description: Workflow for configuring design tooling pipelines, version control
  rules, asset sync scripts, and team workspace environments. Use when executing
  design tooling & workflow pipeline setup workflows, phase transitions, and
  verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Pipeline integration test suite confirms webhook payload processing |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Workflow setup scripts compile without errors |
