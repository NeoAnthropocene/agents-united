---
name: workflow-prototyping-testing-experiment
description: Workflow for building low/medium-fidelity experimental prototypes
  to quickly test hypotheses before full implementation. Use when executing
  rapid prototyping experimentation workflows, phase transitions, and
  verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Prototype link verification test confirms 0 broken interactive pathways |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Prototype code compiles cleanly into preview build |
