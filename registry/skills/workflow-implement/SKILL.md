...
description: End-to-end procedural workflow for code implementation,
  refactoring, and feature execution with TDD validation and regression checks.
  Use when executing implement feature or fix workflows, phase transitions, and
  verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run typecheck --if-present` | 0 type errors across all project files |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run test --if-present` | 100% of unit and integration tests passing cleanly |
...
- **Automated Rollback Protocol**: Execute `git checkout -- .` or `git reset --hard HEAD` to revert incomplete or failing edits if verification fails. If the workspace is not a git repository (`git rev-parse --is-inside-work-tree` fails), restore the pre-change file contents from your own copies or re-apply the inverse edits instead — rollback must never depend on git being present.
