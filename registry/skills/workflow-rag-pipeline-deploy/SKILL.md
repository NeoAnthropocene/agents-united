---
name: workflow-rag-pipeline-deploy
description: End-to-end deployment workflow for Retrieval-Augmented Generation
  systems, vector indexing, hybrid search, and grounding checks. Use when
  executing rag pipeline deployment workflow workflows, phase transitions, and
  verification gates.
metadata:
...
|---|---|---|---|
| Phase 1 -> Phase 2 | Chunks prepared | `npx agents-united doctor` | Chunking schema verified |
| Phase 2 -> Phase 3 | Ingestion complete | `npm run test --if-present` | Vector collection stats confirm 100% vectors indexed |
| Phase 3 -> Completion | Retrieval tested | `npx agents-united doctor` | Grounding checks pass with zero un-cited assertions |
