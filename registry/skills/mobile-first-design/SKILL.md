...
description: Production-grade Mobile First Design playbook for design operations, UX systems, and growth strategy.
metadata:
...
## Overview & Purpose
The Mobile First Design skill provides a deterministic framework for executing mobile first design processes in modern software products.
...
### Execution Triggers
- Direct request to execute Mobile First Design tasks.
- Auditing existing product assets or workflows.
- Standardizing mobile-first-design procedures across team projects.
...
|---|---|---|
| Specification Document | `docs/mobile-first-design/spec.md` | Full specification and guidelines document |
| Component / Asset Files | `src/mobile-first-design/*` | Implemented design tokens, components, or campaign assets |
| Audit Report | `reports/mobile-first-design/summary.json` | Health check and audit metric results |
...
1. Author primary specification document at `docs/mobile-first-design/spec.md`.
...
1. Generate execution summary report at `reports/mobile-first-design/summary.md`.
2. Clean up temporary build artifacts and scratch files.
3. Commit generated files to git repository.
   ```bash
   git add docs/mobile-first-design/ reports/mobile-first-design/
   git commit -m "feat(mobile-first-design): implement Mobile First Design playbook artifacts"
   ```
4. Publish documentation for team review.

## Code & Configuration Exemplars

### Exemplar 1: Mobile First Design Configuration Specification
```yaml
version: "2.0.0"
metadata:
...
rules:
  ...
  reporting:
    format: "json"
    output: "reports/mobile-first-design/summary.json"
```

### Exemplar 2: Mobile First Design Helper Module
```typescript
export function runMobileFirstDesign(scope: string): boolean {
  console.log('Running Mobile First Design on:', scope);
  return true;
}
```
...
### Scenario A: Validation Failure in Mobile First Design
1. **Diagnosis**: Specification or code asset fails validation rules in mobile-first-design.
2. **Recovery Protocol**:
...
### Scenario B: Missing Resource for Mobile First Design
