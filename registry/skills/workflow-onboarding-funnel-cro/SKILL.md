---
name: workflow-onboarding-funnel-cro
description: Workflow for auditing user onboarding drop-off, designing
  activation experiments, optimizing signup flows, and engineering viral
  referral loops. Use when executing product-led onboarding funnel & conversion
  rate optimization workflows, phase transitions, and verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Onboarding test suite validates event tracking triggers and feature flag routing logic |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Frontend UI components and onboarding bundle compile without TypeScript errors |
