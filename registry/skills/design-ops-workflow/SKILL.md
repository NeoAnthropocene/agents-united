...
description: Production-grade Design Ops Workflow playbook for design operations, UX systems, and growth strategy.
metadata:
...
## Overview & Purpose
The Design Ops Workflow skill provides a deterministic framework for executing design ops workflow processes in modern software products.
...
### Execution Triggers
- Direct request to execute Design Ops Workflow tasks.
- Auditing existing product assets or workflows.
- Standardizing design-ops-workflow procedures across team projects.
...
|---|---|---|
| Specification Document | `docs/design-ops-workflow/spec.md` | Full specification and guidelines document |
| Component / Asset Files | `src/design-ops-workflow/*` | Implemented design tokens, components, or campaign assets |
| Audit Report | `reports/design-ops-workflow/summary.json` | Health check and audit metric results |
...
1. Author primary specification document at `docs/design-ops-workflow/spec.md`.
...
1. Generate execution summary report at `reports/design-ops-workflow/summary.md`.
2. Clean up temporary build artifacts and scratch files.
3. Commit generated files to git repository.
   ```bash
   git add docs/design-ops-workflow/ reports/design-ops-workflow/
   git commit -m "feat(design-ops-workflow): implement Design Ops Workflow playbook artifacts"
   ```
4. Publish documentation for team review.

## Code & Configuration Exemplars

### Exemplar 1: Design Ops Workflow Configuration Specification
```yaml
version: "2.0.0"
metadata:
  skill: "design-ops-workflow"
  author: "agents-united"
rules:
  strictValidation: true
  reporting:
    format: "json"
    output: "reports/design-ops-workflow/summary.json"
```

### Exemplar 2: Design Ops Workflow Helper Module
```typescript
export function runDesignOpsWorkflow(scope: string): boolean {
  console.log('Running Design Ops Workflow on:', scope);
  return true;
}
```
...
### Scenario A: Validation Failure in Design Ops Workflow
1. **Diagnosis**: Specification or code asset fails validation rules in design-ops-workflow.
2. **Recovery Protocol**:
...
### Scenario B: Missing Resource for Design Ops Workflow
