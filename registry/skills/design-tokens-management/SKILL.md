...
description: Production-grade Design Tokens Management playbook for design operations, UX systems, and growth strategy.
metadata:
...
## Overview & Purpose
The Design Tokens Management skill provides a deterministic framework for executing design tokens management processes in modern software products.
...
### Execution Triggers
- Direct request to execute Design Tokens Management tasks.
- Auditing existing product assets or workflows.
- Standardizing design-tokens-management procedures across team projects.
...
|---|---|---|
| Specification Document | `docs/design-tokens-management/spec.md` | Full specification and guidelines document |
| Component / Asset Files | `src/design-tokens-management/*` | Implemented design tokens, components, or campaign assets |
| Audit Report | `reports/design-tokens-management/summary.json` | Health check and audit metric results |
...
1. Author primary specification document at `docs/design-tokens-management/spec.md`.
...
1. Generate execution summary report at `reports/design-tokens-management/summary.md`.
2. Clean up temporary build artifacts and scratch files.
3. Commit generated files to git repository.
   ```bash
   git add docs/design-tokens-management/ reports/design-tokens-management/
   git commit -m "feat(design-tokens-management): implement Design Tokens Management playbook artifacts"
   ```
4. Publish documentation for team review.

## Code & Configuration Exemplars

### Exemplar 1: Design Tokens Management Configuration Specification
```yaml
version: "2.0.0"
metadata:
  skill: "design-tokens-management"
  author: "agents-united"
rules:
  strictValidation: true
  reporting:
    format: "json"
    output: "reports/design-tokens-management/summary.json"
```

### Exemplar 2: Design Tokens Management Helper Module
```typescript
export function runDesignTokensManagement(scope: string): boolean {
  console.log('Running Design Tokens Management on:', scope);
  return true;
}
```
...
### Scenario A: Validation Failure in Design Tokens Management
1. **Diagnosis**: Specification or code asset fails validation rules in design-tokens-management.
2. **Recovery Protocol**:
...
### Scenario B: Missing Resource for Design Tokens Management
