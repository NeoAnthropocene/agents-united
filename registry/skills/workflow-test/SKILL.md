---
name: workflow-test
description: Systematic workflow for running unit, integration, and e2e test
  suites, analyzing coverage gaps, and fixing failing tests. Use when executing
  test suite execution & coverage verification workflows, phase transitions, and
  verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | All tests pass without skipped test regressions |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run test:coverage --if-present` | Statement and line coverage meet or exceed configured target percentage |
...
- **Automated Rollback Protocol**: Revert test file modifications via `git checkout -- tests/` if invalid mock setups break test isolation. If the workspace is not a git repository (`git rev-parse --is-inside-work-tree` fails), restore the test files from your pre-change copies or re-apply the inverse edits instead — rollback must never depend on git being present.
