---
name: clickable-prototype-spec
description: Production-grade Clickable Prototype Spec playbook for design operations, UX systems, and growth strategy.
metadata:
...
## Overview & Purpose
The Clickable Prototype Spec skill provides a deterministic framework for executing clickable prototype spec processes in modern software products.
...
### Execution Triggers
- Direct request to execute Clickable Prototype Spec tasks.
- Auditing existing product assets or workflows.
- Standardizing clickable-prototype-spec procedures across team projects.
...
|---|---|---|
| Specification Document | `docs/clickable-prototype-spec/spec.md` | Full specification and guidelines document |
| Component / Asset Files | `src/clickable-prototype-spec/*` | Implemented design tokens, components, or campaign assets |
| Audit Report | `reports/clickable-prototype-spec/summary.json` | Health check and audit metric results |
...
1. Author primary specification document at `docs/clickable-prototype-spec/spec.md`.
...
1. Generate execution summary report at `reports/clickable-prototype-spec/summary.md`.
2. Clean up temporary build artifacts and scratch files.
3. Commit generated files to git repository.
   ```bash
   git add docs/clickable-prototype-spec/ reports/clickable-prototype-spec/
   git commit -m "feat(clickable-prototype-spec): implement Clickable Prototype Spec playbook artifacts"
   ```
4. Publish documentation for team review.

## Code & Configuration Exemplars

### Exemplar 1: Clickable Prototype Spec Configuration Specification
```yaml
version: "2.0.0"
metadata:
  skill: "clickable-prototype-spec"
  author: "agents-united"
rules:
  strictValidation: true
  reporting:
    format: "json"
    output: "reports/clickable-prototype-spec/summary.json"
```

### Exemplar 2: Clickable Prototype Spec Helper Module
```typescript
export function runClickablePrototypeSpec(scope: string): boolean {
  console.log('Running Clickable Prototype Spec on:', scope);
  return true;
}
```
...
### Scenario A: Validation Failure in Clickable Prototype Spec
1. **Diagnosis**: Specification or code asset fails validation rules in clickable-prototype-spec.
2. **Recovery Protocol**:
...
### Scenario B: Missing Resource for Clickable Prototype Spec
