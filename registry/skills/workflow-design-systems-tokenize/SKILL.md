---
name: workflow-design-systems-tokenize
description: Workflow for defining, transforming, and distributing design tokens
  across CSS, Tailwind, TypeScript, and JSON platforms. Use when executing
  design token extraction & transformation workflows, phase transitions, and
  verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run build --if-present` | Style Dictionary compilation builds with exit code 0 |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run test --if-present` | Token validation suite confirms zero undefined token values |
