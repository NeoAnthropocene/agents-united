---
name: workflow-serverless-gpu-deploy
description: Deploying and stress-testing serverless GPU workers on Modal,
  Replicate, and RunPod with cold-start benchmarks and cost safety limits. Use
  when executing serverless gpu deployment workflow workflows, phase
...
|---|---|---|---|
| Phase 1 -> Phase 2 | Guardrails verified | `npx agents-united doctor` | Budget alert and credentials verified |
| Phase 2 -> Phase 3 | Container built | `npm run test --if-present` | Smoke test inference returns 200 OK |
| Phase 3 -> Completion | Metrics validated | `npx agents-united doctor` | Endpoint scales from 0 to N and returns cleanly |
