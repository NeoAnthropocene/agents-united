---
name: workflow-ui-design-color-palette
description: Workflow for defining accessible, semantic color systems, design
  tokens, contrast ratios, and dark mode variants. Use when executing color
  system & palette design workflows, phase transitions, and verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Automated contrast check suite passes for 100% of color token pairs |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Token compilation script builds valid CSS/JSON outputs |
