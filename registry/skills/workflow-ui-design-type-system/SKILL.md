---
name: workflow-ui-design-type-system
description: Workflow for defining typographic scales, font families, line
  heights, font weights, and responsive text sizing. Use when executing
  typography system specification workflows, phase transitions, and verification
  gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Typography scale test verifies mathematical consistency of font steps |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Typography CSS classes compile without syntax errors |
