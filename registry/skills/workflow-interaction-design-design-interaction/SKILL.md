---
name: workflow-interaction-design-design-interaction
description: Workflow for designing component state transitions,
  micro-animations, timing curves, and interactive UI behavior specs. Use when
  executing micro-interaction & transition design workflows, phase transitions,
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Interaction suite verifies prefers-reduced-motion fallbacks for all motion specs |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Motion animation CSS/JS tokens build cleanly |
