---
name: workflow-interaction-design-map-states
description: Workflow for systematically mapping and documenting all UI
  component states (default, hover, focus, active, disabled, loading, error).
  Use when executing component state matrix mapping workflows, phase
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | State matrix test verifies 100% state coverage for all cataloged components |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Component CSS state classes build without compilation errors |
