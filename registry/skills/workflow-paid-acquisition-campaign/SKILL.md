---
name: workflow-paid-acquisition-campaign
description: Workflow for multi-channel PPC campaign architecture, ad copy
  experimentation, creative asset staging, attribution setup, and budget
  guardrails. Use when executing multi-channel paid acquisition campaign
  management workflows, phase transitions, and verification gates.
metadata:
...
| Phase 2 -> Phase 3 | Execution steps complete | `npm run test --if-present` | Ad asset validator confirms 100% adherence to character limits, aspect ratios, and UTM parameters |
| Phase 3 -> Completion | Verification complete & artifacts signed off | `npm run build --if-present` | Campaign launch manifest and tracking configuration compile cleanly |
