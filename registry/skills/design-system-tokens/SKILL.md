...
## Overview & Purpose
The Design System Tokens skill provides a deterministic framework for executing design system tokens processes in modern software products.
...
### Execution Triggers
- Direct request to execute Design System Tokens tasks.
- Auditing existing product assets or workflows.
- Standardizing design-system-tokens procedures across team projects.
...
|---|---|---|
| Specification Document | `docs/design-system-tokens/spec.md` | Full specification and guidelines document |
| Component / Asset Files | `src/design-system-tokens/*` | Implemented design tokens, components, or campaign assets |
| Audit Report | `reports/design-system-tokens/summary.json` | Health check and audit metric results |
...
1. Author primary specification document at `docs/design-system-tokens/spec.md`.
...
1. Generate execution summary report at `reports/design-system-tokens/summary.md`.
2. Clean up temporary build artifacts and scratch files.
3. Commit generated files to git repository.
   ```bash
   git add docs/design-system-tokens/ reports/design-system-tokens/
   git commit -m "feat(design-system-tokens): implement Design System Tokens playbook artifacts"
   ```
4. Publish documentation for team review.

## Code & Configuration Exemplars

### Exemplar 1: Design System Tokens Configuration Specification
```yaml
version: "2.0.0"
metadata:
...
rules:
  ...
  reporting:
    format: "json"
    output: "reports/design-system-tokens/summary.json"
```

### Exemplar 2: Design System Tokens Helper Module
```typescript
export function runDesignSystemTokens(scope: string): boolean {
  console.log('Running Design System Tokens on:', scope);
  return true;
}
```
...
### Scenario A: Validation Failure in Design System Tokens
1. **Diagnosis**: Specification or code asset fails validation rules in design-system-tokens.
2. **Recovery Protocol**:
...
### Scenario B: Missing Resource for Design System Tokens
