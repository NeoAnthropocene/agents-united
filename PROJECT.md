...
| **[0009](./docs/adr/0009-host-conformance-targets.md)** | Host Conformance Targets | Accepted | Per-host conformance probes pinned to tested CLI versions: Cline = conformant (headless CI), Antigravity = interactive-scoped confirmed on agy 1.1.15 (CLI TUI + desktop read `.agents/` natively; headless `-p` not a target). No projection shim required for Antigravity. 2026-09-06: roster subagent frontmatter fully valid on agy 1.1.27 — headless `invoke_subagent` spawn of an exact roster copy succeeded; the Plan 012 desktop failure classified environmental (see ADR addendum). |
