...
- **Repo state at pause**: branch `feat/claude-code-projection`, HEAD `7ebaf65`, clean tree. The Step 0 drift check confirmed `registry/`, `src/`, `tests/`, and `docs/` were untouched since `b9f9a78`; only `plans/`, `CONTEXT.md`, and the new ADR changed.
  ...
  **Gate evidence for the follow-up (run 2026-09-22).** `npm run typecheck` exit 0; `npm test` **43 files / 677 passed / 208 skipped / 0 failed**. Projected proofs: all four engineering specialists carry `SendMessage` with the bare `Agent` pin intact, and all **ten** `digital-agency` projections do too, with zero `send_message` leaks into any projected body. The Cline lane emits no unmapped-tool warnings. Repo scope: only `registry/**`, `src/**`, `tests/**` and `plans/**` changed.
