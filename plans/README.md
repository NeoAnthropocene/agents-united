...
| [023](./023-storeless-installs-and-session-guard.md) | Store-less Claude Installs (ADR 0022) & Plain-Session Guard | Core / Installer / Runtime Integration | **IN PROGRESS — 2026-09-26** (D1–D4 approved; A0 DONE `13f8988` (0.13.0); A1–A3 plain-session guard DONE (#50); B0 inventory DONE; B1–B4 store-less Claude installs DONE — `.claude/.agents-united/` sidecar, upgrade move, teardown, scratch-validated) (A: exec-form guard fix (Windows PowerShell fail-open risk) + consent-gated managed guard entry in `.claude/settings.json` for plain sessions; B: ADR 0022 store-less Claude-only installs via a hidden `.claude/.agents-united/` sidecar snapshot) | 022, ADR 0022 |
