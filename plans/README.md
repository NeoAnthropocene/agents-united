...
| [016](./016-claude-code-projection.md) | Claude Code Projection Architecture — Compound Lane, Launcher Parity & Opt-In Extras (ADR 0018) | Core / Runtime Integration | **IN PROGRESS — Steps 0–1 DONE** at `7ebaf65` (ADR 0018 authored; 7 Claude-lane domain terms registered in `CONTEXT.md`; Step 0 recon census + findings recorded incl. 18-token vocabulary and 5 `generative_ui` references; `tsc` clean). **Paused before Step 2 (Red tests)** pending Cline subagent re-authentication — see the plan's § Resumption notes. **v1 rollout: Claude only** | 007, 008, 013, 015 |
