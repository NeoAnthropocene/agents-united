...
| Kimi / Moonshot host projection — listed on the TUI as 🗓 Planned (display only: the `.kimi/` layout is UNVERIFIED and is never stubbed, per Plan 017 R3). Its projector is planning-list work: a `HostDialectSpec` entry to be authored after the Plan 017 codex lands — entry point is Plan 017 Step 0's Kimi / Moonshot docs spike | CLI / Runtime Integration | Directed 2026-09-22: product focus is Antigravity, Cline and Claude Code; cursor / opencode / codex are TUI-tagged "Under Development"; Kimi / Moonshot is the next new host — listed, not implemented |
